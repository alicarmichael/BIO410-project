...
Bee_richness_change <- Bee_richness.after - Bee_richness.before
...
Now we will conduct our simulation using A/B testing.
```{r}
# Shuffle the labels of the Site.status column
shuffled_status <- sample(final_oba_df$Site.status)

# Make a new shuffled labels column to our data set
final_oba_df$Shuffled.status <- shuffled_status

# Simulate the statistics above with the shuffled labels for the bees
null_simulation_bee <- replicate(1000,{
  shuffled_status <- sample(final_oba_df$Site.status)
  Richness_before_bees <- length(unique(final_oba_df$Bee.genus[final_oba_df$shuffled.status == "Before"]))
  Richness_after_bees <- length(unique(final_oba_df$Bee.genus[final_oba_df$shuffled.status == "After"]))
  Richness_after_bees - Richness_before_bees
  
})

# Create our observed value using the richness change calculated above
observed <- Bee_richness_change

# Calculate our p_value simualted under the null hypothesis
p_value <- mean(null_simulation_bee >= observed)
print(p_value)
```
...
# Simulate the statistics above with the shuffled labels for the plants
null_simulation_plant <- replicate(1000,{
  shuffled_status <- sample(final_oba_df$Site.status)
  Richness_before_plants <- length(unique(final_oba_df$Plant.genus[final_oba_df$shuffled.status == "Before"]))
  Richness_after_plants <- length(unique(final_oba_df$Plant.genus[final_oba_df$shuffled.status == "After"]))
  Richness_after_plants - Richness_before_plants
  
})

observed <- Plant_richness_change
p_value <- mean(null_simulation_plant  >= observed)
print(p_value)
```
