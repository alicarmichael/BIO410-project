...
Creating the 6km buffer around the three fires we will be using
```{r}
...
Creating a new Column for the site status to determine if the observation was done before or after the fire 
```{r}
...
for (i in 1:nrow(oba)) {
  if (oba$Year.1[i] < 2020) { # Less than 2020 is equivalent to before the fires
    oba$Site.status[i] <- "Before Fire"
  }
  else if (oba$Year.1[i] > 2020) { # Greater than 2020 is equivalent to after the fires
    oba$Site.status[i] <- "After Fire"
  } else { # 2020 gets the status of "During the fire"
    oba$Site.status[i] <- "During Fire"
  }
}

# Inspect the column created
head(oba)
```

Subset the oba dataframe to just the observations that are in the buffer of our fires
```{r}
oba_and_fire_df <- oba %>% 
  select()
```
